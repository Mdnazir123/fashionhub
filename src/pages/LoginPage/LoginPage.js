...
// var passwordRegex = new RegExp('');
 const loginSchema = yup.object({
    email: yup.string().email('Please enter your valid username.').required('Please enter your valid username.'),
...
const LoginPage = () => {
  ...
  return (
    <div className="cm-register loginpage p-3 md:p-4">
              ...
              <div className="form-wraper p-4">
                  ...
                  <label for="email">Username</label>
                     ...
                     {errors.password && touched.password ? (<p className='form-error'>{errors.password}</p>) : null }
                      <button type='submit' className="hs-button w-full max-w-[150px] m-auto bg-red-500 hover:bg-red-600 cursor-pointer text-white text-xl font-medium text-center py-1 rounded-full mt-4">Login</button>
                  </form>
                  <p className="text-left text-sm mt-2">Don't  have account ? <Link className="text-red-500 underline" to="/register">Sign Up</Link></p>
              </div>
          </div>
      </div>
  )
